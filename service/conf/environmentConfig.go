...
type Params struct {
	Address              string `id:"address" short:"a" default:"0.0.0.0:50541" desc:"Listening address"`
	...
	if params.Config == "" {
		if params.Lite {
			if userConfigDir, e := os.UserConfigDir(); e == nil {
				params.Config = filepath.Join(userConfigDir, "v2rayb")
			} else {
				params.Config = "$HOME/.config/v2rayb"
			}
		} else {
			params.Config = "/etc/v2rayb"
		}
	}
